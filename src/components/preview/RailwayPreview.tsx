  ...
  if (status !== 'ready') {
    return (
      <div className={`flex flex-col ${className}`}>
        <div className="flex flex-col items-center justify-center p-8 bg-slate-800 rounded-lg flex-1">
          {/* Railway logo/icon */}
          <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-gold-600 to-pink-600 flex items-center justify-center mb-6 shadow-lg">
          ...
          <div className="w-64 h-2 bg-slate-700 rounded-full overflow-hidden mb-4">
            <div
              className="h-full bg-gradient-to-r from-gold-500 to-pink-500 rounded-full transition-all duration-500 animate-pulse"
              style={{
                width:
                  status === 'creating'
                    ? '25%'
          ...
          <div className="text-slate-300 text-sm font-medium">{getStatusMessage()}</div>
...
function StatusStep({
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className={`text-xs ${active || completed ? 'text-slate-300' : 'text-slate-500'}`}>
        {label}
      </span>
    </div>
  );
...
function BuildLogsPanel({
  ...
  if (!isOpen) {
    return (
      <button
        onClick={onToggle}
        className="mt-2 text-slate-400 hover:text-slate-200 text-xs py-1 flex items-center gap-1"
      >
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="relative mt-2">
      <button
        onClick={onToggle}
        className="absolute top-1 right-1 z-10 text-slate-400 hover:text-slate-200 text-xs px-2 py-0.5 bg-slate-800 rounded"
      >
        Hide
      </button>
      <div className="h-32 bg-slate-900 rounded-lg overflow-hidden">
